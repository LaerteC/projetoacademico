    ...
    <body class="container" style="background-color:#F5F5DC;">
        ...
        <form action="${pageContext.request.contextPath}/${redirecionarFormulario}&id=${id}" method="Post">
            <div class="form-group">
                <label for="exampleInputEmail1">Nome do Ingrediente</label>
                <input 
                    type="text"
                    style="width:600px;"
                    maxlength="100"
                    ...
                    id="nome"
                    autocomplete="off"
                    value="<c:out value="${ingrediente.nomeIngrediente}"/>" required="true">
            </div>

            <div class="form-group">
                <label for="exampleFormControlSelect1">Tipo de Ingrediente</label>
                <select
                    name="idTipoIngrediente"
                    class="form-control" 
                    id="idTipoIngrediente"
                    style="width:300px;"
                    >
                    <c:forEach items="${tipoIngrediente}" var="tipo" >                       
                        <option value="${tipo.idTipoIngrediente}" ${(tipo.idTipoIngrediente == ingrediente.idTipoIngrediente) ? 'selected' : ''}>
                            ${tipo.nomeTipoIngrediente}
                        </option>
                    </c:forEach>
            ...
            <button 
                ...
                >
                ${(empty ingrediente.nomeIngrediente) ? "Salvar" : "Alterar" }
            </button>
            <a
