    ...
    <body class="navbar container">
            ...
            <table class="table">
                ...
                <c:forEach var="gerente" items="${gerentes}">
                    <tr>                        
                        <td>${gerente.nome}</td>
                        <td>${gerente.cpf}</td>
                        <td><fmt:formatDate type="date" pattern="dd/MM/yyyy" value="${gerente.dataNascimento}"/></td>
                        <td>${gerente.email}</td>
                        <td>
                            <a 
                                style="background-color:#027700;"
                                class="btn btn-primary btn-custom" 
                                href="${pageContext.request.contextPath}/GerenteServlet?action=formUpdateGerente&id=${gerente.id}" 
                                role="button"
                            >
                            Edit
                            </a>
                            <button
                                ...
                                class="btn btn-primary btn-custom " 
                                value="Delete"
                                onclick="confirmacao(${gerente.id})"
                            >
                             Del
                            </button>
                            <a 
                                style="background-color:#027700;"
                                class="btn btn-primary btn-custom" 
                                href="${pageContext.request.contextPath}/GerenteServlet?action=showGerente&id=${gerente.id}" 
                                role="button"
                            >
                            Ver
                            </a>
       ...
       <div>
        <a 
          ...
          style="background-color:#027700;" 
          href="${pageContext.request.contextPath}/GerenteServlet?action=formNewGerente"
        >
            Novo Gerente
        </a>        
        <a 
     ...
     <%-- JAVA SCRIPT --%>
        <script type="text/javascript">
            ...
            function confirmacao(idExcluir){
                ...
                ajax.open(
                    "GET",
                    "${pageContext.request.contextPath}/GerenteServlet?action=desativarGerente&id="+idExcluir);
                    ajax.send();                                 
              }
            }         
