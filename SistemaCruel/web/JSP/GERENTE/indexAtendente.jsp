            ...
            <table class="table">
                ...
                <c:forEach var="atendente" items="${atendentes}">
                    <tr>                        
                        ...
                        <td>${atendente.cpf}</td>
                        <td><fmt:formatDate type="date" pattern="dd/MM/yyyy" value="${atendente.dataNascimento}"/></td>
                        <td>${atendente.email}</td>
                        <td>
                            <a 
                                style="background-color:#027700;"
                                class="btn btn-primary btn-custom" 
                                href="${pageContext.request.contextPath}/GerenteServlet?action=formUpdateAtendente&id=${atendente.id}" 
                                role="button"
                            >
                            Edit
                            </a>
                            <button
                                ...
                                class="btn btn-primary btn-custom " 
                                value="Delete"
                                onclick="confirmacao(${atendente.id})"
                            >
                             Del
                            </button>
                            <a 
                                style="background-color:#027700;"
                                class="btn btn-primary btn-custom" 
                                href="${pageContext.request.contextPath}/GerenteServlet?action=showAtendente&id=${atendente.id}" 
                                role="button"
                            >
                            Ver
                            </a>
